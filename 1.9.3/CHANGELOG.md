...
- ✅ **待办事项导出**：在待办列表中新增导出按钮，可一次导出当前列表内全部已完成和未完成任务。
- 📝 **Markdown 兼容文本**：导出为 UTF-8 with BOM 的 `.txt` 文件，使用 `- [ ]` / `- [x]` 保留完成状态，并在存在到期日时附加日期。
- 🌐 **导出内容国际化**：按钮文字、到期日标签以及无标题时的默认文件名随游戏语言切换，支持中文、英文和日文。
- 🎨 **原生界面适配**：导出按钮使用游戏原生纯文字胶囊样式，放置在“新任务”右侧，避免挤压或遮挡待办列表。
- ✅ **回归测试**：补充导出格式、文件写入、顺序、状态、到期日和本地化测试。

### v1.9.2 - 后台省电判定与笔记导出体验优化

- 🪟 **后台省电按实际可见性触发**：窗口失焦后不再立即降级；仅在最小化、切到其他虚拟桌面、完全移出屏幕或被同显示器全屏窗口遮挡时进入后台模式，多显示器场景不再误判。
- 🐛 **窗口可见性修复**：修复全屏窗口层级误判、Alt+Tab 宽限期被轮询绕过、DWM cloaked 窗口被当作有效遮挡，以及窗口仅与屏幕边缘相切时仍被判为可见的问题。
- ⚡ **后台检测降噪与降耗**：获焦时立即恢复前台模式，后台状态降低轮询频率，不可见诊断日志仅在状态切换时输出。
- 📝 **笔记导出界面优化**：导出/取消/确认按钮改为与新增笔记按钮纵向排列，并为笔记列表自动预留空间，避免顶部控件拥挤或遮挡。
- 🎨 **笔记选择样式统一**：多选标记改用游戏原生圆形勾选图标，替代文本式 `[ ]` / `[x]` 标记。
- 🔧 **构建与兼容性维护**：主项目迁移到 SDK 风格，使用 Assembly Publicizer 直接访问游戏 API，移除易碎的反射类型查找，并补充相关策略与纯逻辑测试。

### v1.9.1 - MOD 设置条件可见性 API
...
- 🥔 **三级渲染模式**：正常 / 土豆(F2) / 后台(失焦自动)，三级独立保存/恢复渲染参数
  - 后台省电优化：窗口失焦自动限帧 10fps + 渲染分辨率 0.1 + 关闭阴影和垂直同步，获焦自动恢复
  - PiP 豁免：小窗期间跳过后台优化，退出后按焦点状态恢复策略
  - 土豆模式优先级最高，不受焦点切换影响
...
- 🎨 **MOD 设置面板重构**：从 Credits 切换到 General 模板，彻底解决布局偏移问题
  ...
  - 下拉层新增智能排序层级过渡控制
- 🐛 **缺陷修复**：服装悄悄话输入空值防护、日志级别统一降噪

### v1.8.1 - 原版设置激活态修复与发版流程改进

- 🐛 **设置界面修复**：修复安装 mod 后原版设置中 ON/OFF 激活高亮丢失的问题（仅重置真正的 tab `InteractableUI`，不再误伤具体设置项）。
- ✅ **回归测试**：新增 `SettingUITabFieldSelectorTests`，防止后续再出现“反射字段筛选过宽”导致的 UI 激活态回归。
- 🧩 **版本管理统一**：新增仓库根目录 `version.json` 作为单一版本号来源，并提供 `scripts/sync-version.ps1` 一键同步核心版本文件与文档标题。

### v1.8.0 - 笔记导出与缺陷修复版

- 📝 **笔记多选导出**：支持在游戏内笔记界面多选笔记，一键导出为 `.txt` 文件（UTF-8 with BOM），通过 Windows 原生文件夹选择对话框指定输出目录。
- 🗑️ **删除确认**：笔记删除操作增加确认弹窗，防止误删，可在 MOD 设置中开关。
- 🌐 **全量 i18n 覆盖**：导出选择模式 UI、删除确认 UI 均完成中/英/日三语翻译接入。
- 🐛 **缺陷修复**：修复镜像模式下错误激活 UI 按钮的问题；修复代码审阅发现的逻辑缺陷。
- 📚 **文档整顿**：重组 `docs/` 目录结构，整理 MOD 设置对接指南，清理过期内部开发文档。
